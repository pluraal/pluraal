...
program
    ...
    .option("-d, --definition <names...>", "Only run tests for these definition names")
    ...
    if (reporter === "junit") {
        ...
        console.error(`Unknown reporter: "${opts.reporter}". Supported: text, junit`);
        process.exit(1);
    }
    if (opts.output) {
...
program
    ...
    .option("-i, --input <pairs...>", "Input values as key=value pairs (e.g. --input unit_price=10 quantity=3)")
    .action(async (file, definitionName, opts) => {
    const filePath = resolve(process.cwd(), file);
    let module;
    try {
        module = await parseUserModule(filePath);
